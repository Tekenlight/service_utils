...
service_client.transceive_using_client = function(context, inp, client)
    ...
    local s, obj = true, nil;
    if (require('pl.stringx').strip(response_json) == '{}') then
        obj = {};
    elseif (response_json ~=nil) then
        s, obj = service_client.prepare_response_obj(context, method_properties, response_json);
        assert(s, obj);
    end
