...
local _factory = {};
eh_cache.add('{http://evpoco.tekenlight.org/idl_spec}interface', _factory);



function _factory:new_instance_as_root()
    local type_handler = basic_stuff.get_element_handler('http://evpoco.tekenlight.org/idl_spec', 'interface_type');
    local obj =  type_handler:new_instance_as_global_element({
                                        ns = 'http://evpoco.tekenlight.org/idl_spec',
                                        local_name = 'interface',
                                        generated_name = 'interface',
                                        root_element = true,
                                        min_occurs = 1,
                                        max_occurs = 1});
    return obj;
