...
local _factory = {};
eh_cache.add('{http://evpoco.tekenlight.org}inbound_email_message', _factory);



function _factory:new_instance_as_root()
    local type_handler = basic_stuff.get_element_handler('http://evpoco.tekenlight.org', 'inbound_email_message_type');
    local obj =  type_handler:new_instance_as_global_element({
                                        ns = 'http://evpoco.tekenlight.org',
                                        local_name = 'inbound_email_message',
                                        generated_name = 'inbound_email_message',
                                        root_element = true,
                                        min_occurs = 1,
                                        max_occurs = 1});
    return obj;
