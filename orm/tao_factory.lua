...
tao.raw_update = function(self, context, obj, col_map, all_columns)
    if (all_columns == nil) then all_columns = false; end
    assert(type(all_columns) == 'boolean');
    assert(context ~= nil and type(context) == 'table');
    assert(obj ~= nil and type(obj) == 'table');
    if (col_map ~= nil) then
        assert(type(col_map) == 'table');
...
tao.update_all_columns = function(self, context, obj)
    ...
    return self:raw_update(context, obj, col_map, true);
end
