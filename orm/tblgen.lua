...
for i, v in ipairs(tbl_struct.columns.column) do
    ...
    if (v._attr.default_value ~= nil) then
        if (v._attr.memory_data_type == nil) then
            error("memory_data_type not specified for column - ".. v._attr.name);
        else
            local stat, dv = pcall(primitives.new, v._attr.memory_data_type, v._attr.default_value);
            if (not stat) then
                error(dv);
            end
        end
    ...
    tbl_def.declared_columns[v._attr.name].memory_data_type = v._attr.memory_data_type
