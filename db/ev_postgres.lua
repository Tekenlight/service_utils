...
local cur_mt = { __index = ev_postgres_cursor };
local cur_r_mt = { __index = ev_postgres_cursor_res };
...
ev_postgres_stmt.map = function(q_out, fields, sng_lvl)
    if (q_out == nil) then
        return nil;
    end
    if (sng_lvl == nil) then
...
ev_postgres_conn.open_cursor = function(self, cursor_id, i_sql_stmt, ...)
    ...
    local cursor = { _conn = self, _cursor_id = cursor_id };
    cursor = setmetatable(cursor, cur_mt);
    assert(cursor ~= nil);
...
ev_postgres_cursor.fetch_all = function(self)
    ...
    assert(cur_mt == getmetatable(self));
    local sql_stmt = "FETCH ALL in " .. self._cursor_id ;
    local stmt = self._conn:prepare(sql_stmt);
    stmt:execute();
    local cu_res = setmetatable({_stmt = stmt, _cur = self}, cur_r_mt);
...
ev_postgres_cursor_res.map = function(self, rec, map, sng_lvl)
    ...
    return self._stmt.map(rec, map, sng_lvl);
end
